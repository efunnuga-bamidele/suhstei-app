...
export default function ClosedRequestPage(){
    ...
    const currentItems = AllBookRequest.filter((items) => items.request_status === "Closed").slice(indexOfFirstPost, indexOfLastItem);
    const pageNumber = Math.ceil((AllBookRequest.filter((items) => items.request_status === "Closed").length) / itemsPerPage);
        ...
        <div className='bg-gray-100 mx-1 font-body scroll-smooth h-0'>
            ...
            <main className="bg-gray-300 mt-5 flex flex-wrap-reverse md:flex-nowrap overflow-x-hidden">
                ...
                <section className="bg-white mt-12 m-2 p-2 w-full rounded-md relative overflow-x-auto">
                        <h1 className="font-bold text-lg text-center underline text-gray-600">Closed Book Requests</h1>
                        ...
                        {AllBookRequest && AllBookRequest.filter((items) => items.request_status === "Closed").length === 0  && (
